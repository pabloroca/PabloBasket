...
open class BasketDataControler: DefaultDataManager {
    ...
    open func readforKey(fkid: Int) -> Basket? {
        if let basket = readFirstFor(objectype: Basket.self, predicate: NSPredicate(format: "fkid = %d", fkid)) as? Basket {
            return basket
        } else {
            return nil
        }
    ...
    open func update(fkid: Int, units: Int) {
        do {
            try realm.safeWrite {
                if let basket = readforKey(fkid: fkid) {
                    basket.units = units
                } else {
                    realm.add(Basket.init(fkid: fkid, units: units)!)
                }
            }
    ...
    open func calculateBasket() -> Double {
        let arrBasket = readAll(objectype: Basket.self) as? [Basket] ?? []
        var total: Double = 0
    ...
    open func readAll() -> [Basket] {
        return readAll(objectype: Basket.self) as? [Basket] ?? []
    }
    
    open func deleteAll() {
        do {
            try realm.safeWrite {
                realm.delete(readAll(objectype: Basket.self))
            }
        } catch let error as NSError {
